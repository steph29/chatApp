...
class ProfilController extends StatefulWidget{
	...
	ProfilControllerState createState() => ProfilControllerState();
}
...
class ProfilControllerState extends State<ProfilController>{
	...
	Future<void> _takePicture(ImageSource source) async {
		...
		// obtenir une url apres avoir ajouter image dans le stockage.
		FirebaseHelper().savePicture(image as File, FirebaseHelper().storage_user.child(widget.id)).then((string) {
			Map map = user.toMap();
			map["imageUrl"] = string;
			FirebaseHelper().addUser(user.id, map);
			_getUser();
	...
	Future<void> _logOut(BuildContext context) async {
		...
		Text subTitle = new Text("Êtes-vous sur?");
		return showDialog(
				context: context,
